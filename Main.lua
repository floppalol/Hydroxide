...
-- =========================
-- OUR CUSTOM SERVER NUKE (button works 100%)
-- =========================

local nukeActive = false

-- This makes the button actually appear and work
local nukeBtn = playerPage:AddButton("Our Server Nuke: OFF", function()
    nukeActive = not nukeActive
    nukeBtn:SetText("Our Server Nuke: " .. (nukeActive and "ON" or "OFF"))
    
    if nukeActive then
        print("[NUKE] Server is getting destroyed...")
    else
        print("[NUKE] Stopped.")
    end
end)

-- The actual nuke (runs only when button is ON)
task.spawn(function()
    while task.wait(0.03) do
        if not nukeActive then continue end
        
        for i = 1, 35 do
            task.spawn(function()
                local p = Instance.new("Part")
                p.Size = Vector3.new(60, 60, 60)
                p.Transparency = 1
                p.Anchored = false
                p.CanCollide = false
                p.CFrame = CFrame.new(math.random(-500,500), 10000,math.random(-500,500))
                p.Parent = workspace
                
                p:SetNetworkOwner(nil) -- server owns = replicates to everyone
                
                p.AssemblyLinearVelocity = Vector3.new(
                    math.random(-120000,120000),
                    math.random(80000,200000),
                    math.random(-120000,120000))
                )
                p.AssemblyAngularVelocity = Vector3.new(1000,1000,1000)
